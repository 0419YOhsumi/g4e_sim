...
class Geant4Eic(object):
    def __init__(self, **kwargs):
        ...
        self.commands = []         # Total list of commands, generated by g4epy + 
        ...
        self.config['pgun_name'] = self.config.get('pgun_name', 'cone_pgun_my')
    ...
    def macro(self, files):
        ...
        for file in files:
            ...
            self.commnand(f"/control/execute cone_pgun_my.mac")
    ...
    def source(self, source_file):
        ...
        mc_type = detect_mc_type(source_file) # chnage 12/14
    ...
    def generate_run_mac(self):
        ...
        pgun_name = self.config['pgun_name']
        #base_mac_file = pgun_name
        base_mac_file = det_name if is_batch else det_name + 'vis'
        base_mac_file += '.mac'

        self.commands.append(f"/control/execute {base_mac_file}")
        #self.commands.append(f"/control/execute cone_pgun_my.mac")
        self.commands.append(f"/detsetup/beamlineName {self.config['beamline']}")
        self.commands.extend(self.user_commands)
        #self.commands = [f"/generator/select coneParticleGun"]
        self.commands = [f"/generator/select particleGun"]
        
        #self.commands.append(f"/tracking/verbose 1")
        self.commands.append(f"/gps/verbose 1")
        
        self.commands.append(f"/gps/particle pi0")
        self.commands.append(f"/gps/pos/type Beam")
        self.commands.append(f"/gps/energy 50 GeV")
        self.commands.append(f"/gps/ene/type Gauss")
        self.commands.append(f"/gps/ene/sigma 5 MeV")
       
        self.commands.append(f"/gps/pos/parthe pi rad")
        self.commands.append(f"/gps/pos/centre 0. 0. 0. m")
        self.commands.append(f"/gps/pos/parphi pi rad")
        
        #self.commands.append(f"/gps/direction 0.0256 0. 0.999")
        #self.commands.append(f"/gps/ang/rot1 0.999 0 -0.0256")
        self.commands.append(f"/gps/ang/rot1 1 0 0")
        self.commands.append(f"/gps/ang/rot2 0 -1 0")

        self.commands.append(f"/gps/ang/type focused")
        self.commands.append(f"/gps/ang/focuspoint 0.96 0. 37.5 m")
        
        #self.commands.append(f"/gps/ang/sigma_x 0.001 rad")
        #self.commands.append(f"/gps/ang/sigma_y 0.001 rad")
        #self.commands.append(f"/gps/ang/sigma_r 0.006 rad")
        
        #self.commands.append(f"/gps/ang/maxtheta 0.0316 rad")
        #self.commands.append(f"/gps/ang/mintheta 0.0196 rad")
        #self.commands.append(f"/gps/ang/minphi 2.90759 rad")
        #self.commands.append(f"/gps/ang/maxphi 3.37559 rad")
        
        self.commands.append(f"/run/initialize")
        self.commands.append(f"/run/beamOn 1000") 
    ...
    #def generator_skip(self, value):
        #self.command("/run/initialize")
        #self.command(f"/generator/skipEvents {value}")
        #return self
